...
function createServer(config?: { apiKey?: string; apiUrl?: string }) {
  const API_URL = config?.apiUrl || process.env.NEEDHUMAN_API_URL || "http://localhost:3000";
  const API_KEY = config?.apiKey || process.env.NEEDHUMAN_API_KEY || "";
...
// Smithery sandbox export — allows tool scanning without real credentials
export function createSandboxServer() {
  return createServer({ apiKey: "sandbox", apiUrl: "https://needhuman.ai" });
}
